...
│   ├── NoteActionsRow.jsx   # Desktop copy/delete icon row, rendered below a note only while `note.id === editingNoteId` and it's not the perpetual last note — see Patterns: "Note delete & copy actions" below
│   ├── NoteActionsSheet.jsx # Mobile equivalent of NoteActionsRow, opened by a long-press (useLongPress) on any non-last note; follows the `usePresence`/`.anim-sheet` floating-overlay template
│   ├── extensions/
...
│   └── useLongPress.js      # Touch-only long-press detection (ignores mouse pointerType), spread onto NotebookFeed's `.entry-block` rows to open NoteActionsSheet; suppresses the synthetic click a touch browser fires after long-press-release so it doesn't also trigger click-to-edit
├── lib/
...
- **Merge via `Map` keyed by note id** (`db.js:getNotes`): start from Supabase results, then overlay local IndexedDB notes — `pending` local always overwrites remote, `pending-delete` local removes any remote counterpart from the result (see "Note delete & copy actions" below), `synced` local without a remote counterpart is preserved (assumed sync-lag, not a delete), `synced` local that matches remote is left alone.
...
- **Note delete & copy actions (2026-07-29)** — desktop: `NoteActionsRow` renders below a note only when it's the actively-edited one (`note.id === editingNoteId`, NOT the scroll-driven `activeNoteId`) and it's not the perpetual last note (`!isLast` — that note never gets an action row at all, matching its existing exemption from delete-on-empty-blur). Mobile: `useLongPress` handlers are spread onto every non-last `.entry-block`; firing opens `NoteActionsSheet` with the resolved note (loose `==` lookup from `data-note-id`, same convention as the IntersectionObserver below). Delete always goes through a single shared `AlertDialog` in `NotebookFeed.jsx` (`pendingDeleteNote` state) — the desktop row opens it directly, but the mobile sheet defers opening it until its own close transition finishes (`onTransitionEnd`), since the sheet is `z-[60]` and the dialog overlay is `z-50` and would otherwise render underneath a still-animating sheet. Copy is plain `navigator.clipboard.writeText(note.content)` — note content is always plain text (`TiptapEditor.jsx` calls `editor.getText()` before every save), so no HTML/ProseMirror-JSON extraction is needed. Deleting the currently-`editingNoteId`/`activeNoteId` note explicitly clears both (`NotebookFeed.jsx`'s `confirmDelete`, `NoteApp.js`'s `onDeleteNote` prop) rather than leaving them pointed at a removed note.
- **Pending-delete queue (2026-07-29)** — `deleteNote()` in `db.js` used to `throw` on any Supabase failure with no offline path; now, on failure, it writes the note back locally with `syncStatus: 'pending-delete'` (same IndexedDB `syncStatus` index used by `'pending'`/`'synced'`, no schema/version change) and still returns success — the caller's optimistic local removal always sticks. `syncPendingDeletes()` (mirrors `syncPendingNotes()`'s shape) retries these on the next `window.online` event, wired in `useNotes.js` alongside the existing `syncPendingNotes` call. A temp-UUID note id (never synced to Supabase in the first place) is purged locally with no Supabase call at all, rather than being queued. **Non-goal:** this only protects the *deleting device's own* pending deletes from `getNotes()`'s merge — it does not add cross-device tombstones, so a note hard-deleted online by Device A can still be resurrected on Device B if B held a stale local `synced` copy from before the delete (pre-existing gap, see the Merge bullet above and `db.js`'s `getNotes` comment).
- **Note appear/hide animation (2026-07-29)** — three additions to the floating-overlay pattern above, all following the same "no JS mount dance" philosophy via CSS `@starting-style`: (1) `NoteActionsRow` is now rendered for every non-last note (not conditionally mounted) and takes an `isOpen` prop; it owns its own `usePresence(isOpen)` internally and drives a new `.anim-reveal[data-state]` class (`globals.css`). Since this row lives in normal document flow (it sits below the note, pushing the next one down), a plain opacity fade wasn't enough — the box still claimed full height the instant it mounted, so the note below jumped in one frame. `.anim-reveal` instead animates `grid-template-rows: 0fr -> 1fr` (the no-JS auto-height-reveal trick), which requires the specific two-level DOM structure in `NoteActionsRow.jsx` (outer grid container -> single grid item with `overflow: hidden` -> the actual button row) — the `overflow: hidden` wrapper is what lets the 0fr state actually clip content instead of being clamped by its min-content size. This lets the *previous* editingNoteId's row finish its own exit collapse while the *new* one grows in, since each is an independent component instance. (2) A newly-created note (blur-created blank note, jump-to-latest, gutter click) gets a `.note-enter` class on its `.entry-block`, which pairs with a bare `@starting-style` rule (opacity 0 → 1, translateY 8px → 0) — no `data-state`/`usePresence` needed since `@starting-style` only ever fires once, at the DOM node's first insertion. Which notes count as "new" is tracked by `NotebookFeed.jsx`'s `seenNoteIdsRef`, seeded synchronously with every note id present on the very first render (so the initial batch, possibly hundreds of notes, never animates) and updated in a `useEffect` after each commit. (3) Deleting a note plays a shrink-and-fade (`opacity`→0, `scale`→0.96) via `.entry-block[data-state="closed"]` before the row actually unmounts — since `removeNote` filters the note out of `notes` synchronously, `NotebookFeed`'s `deletingNote` snapshot + `displayNotes` memo re-insert it at its original sorted position for exactly the duration of that transition, then `handleDeleteExitEnd` (gated on `e.propertyName === 'transform'` to fire only once) clears the snapshot. **Constraint that matters if this ever needs touching:** `.entry-block`'s transition is declared directly in `globals.css` (not via a Tailwind `transition-*` utility in the className) — Tailwind's utilities layer would otherwise outrank the custom `@layer components` declaration and silently cancel it.
...
- `syncPendingNotes()` and `syncPendingDeletes()` in `db.js` (both run on `window.online`, wired in `useNotes.js`)
- `getNotes()` merge logic in `db.js` — check `syncStatus` on the local record (`'pending'` / `'pending-delete'` / `'synced'`)
