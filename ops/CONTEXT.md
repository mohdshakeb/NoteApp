...
- **Backend:** Supabase — Postgres (`notes` table) + Auth (Google OAuth, magic-link email). Row Level Security (RLS) enforced at the database level: users can only access rows where `user_id` matches their own id. Versioned schema: `supabase/schema_snapshot.sql` — table definition, RLS policies, and the `delete_user()` account-deletion function, all confirmed directly against the live database via SQL Editor introspection queries on 2026-09-15 (not guessed). `supabase/config.toml` exists (`supabase init` already run) but the project isn't linked yet — `db pull`/`db dump` need Docker or Podman, neither installed on the dev machine as of this writing. See the header of `schema_snapshot.sql` for the exact commands to finish that setup and get a real CLI-tracked migration once Docker/Podman is available; until then, keep the snapshot file updated by hand if the schema changes.
